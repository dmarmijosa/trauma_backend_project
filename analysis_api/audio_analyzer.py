import datetime # Asegúrate de que esté importado
import tempfile
import os
import time
import ffmpeg 
...
from pdf_services.services import PdfService # Ajusta la ruta si es necesario
...
class AudioAnalyzer:
    ...
    def _initialize_yamnet_classifier_globally(cls):
        ...
        try:
            ...
            # Ruta al modelo YAMNet. Asegúrate de que esta ruta sea correcta
            # y que el archivo exista. Se asume que 'assets/models/' está en la raíz del proyecto.
            model_path = str(Path(os.getcwd()) / 'assets' / 'models' / 'yamnet.tflite')
            # O si lo dejaste en la raíz del proyecto:
            # model_path = str(Path(os.getcwd()) / 'yamnet.tflite')


            if not os.path.exists(model_path):
                cls._class_logger.error(f"YAMNet model TFLite no encontrado en {model_path}. Descárgalo y colócalo en la ruta correcta (ej. 'assets/models/yamnet.tflite' en la raíz de tu proyecto).")
                cls._class_logger.error("URL de descarga: https://storage.googleapis.com/mediapipe-models/audio_classifier/yamnet/float32/1/yamnet.tflite")
                cls.yamnet_init_error = FileNotFoundError(f"YAMNet model not found: {model_path}")
                return
    ...
    def __init__(self):
        ...
        AudioAnalyzer._initialize_yamnet_classifier_globally() # Llama al método de clase
        try:
    ...
    def decode_and_prepare_audio(self, input_file_path: str, original_filename: str) -> tuple[str | None, int | None]:
        # ... (Sin cambios respecto a la versión anterior que funcionaba con FFmpeg)
    ...
    def extract_acoustic_features(self, wav_audio_path: str, sr: int) -> ExtractedFeatures | None:
        # ... (Sin cambios respecto a la versión anterior que funcionaba con Librosa) ...
    ...
    def analyze_with_yamnet(self, wav_audio_path: str, sample_rate: int) -> YamnetResult | None:
        ...
        try:
            # Cargar audio con Librosa para obtener el array NumPy
            y, sr_loaded = librosa.load(wav_audio_path, sr=sample_rate) # sr debe ser 16000
            if sr_loaded != 16000: # Doble verificación
                 self.logger.error(f"AudioAnalyzer_PY_YAMNET_ERROR: Librosa cargó el audio con SR={sr_loaded} en lugar de 16000Hz.")
                 return {"topClasses": [], "notes": f"Sample rate incorrecto para YAMNet después de carga con Librosa: {sr_loaded}Hz."}


            # CORRECCIÓN: Usar create_from_array con argumentos posicionales
            audio_clip_data = mediapipe_containers.AudioData.create_from_array(
                y, # El array NumPy de samples
                sr_loaded # El sample rate (debería ser 16000)
            )
            ...
            if classification_result_list and len(classification_result_list) > 0:
                # Para AUDIO_CLIPS, la documentación de MediaPipe indica que classify devuelve AudioClassifierResult, no una lista.
                # Sin embargo, si devuelve una lista, el primer elemento es el relevante para el clip completo.
                # Vamos a manejar ambos casos verificando el tipo.
                # Asumiremos que es AudioClassifierResult directamente o el primer elemento de una lista.
    ...
    def run_crepe_pitch_analysis(self, wav_audio_path: str, sr: int) -> CrepePitchResult | None:
        self.logger.warn(f"AudioAnalyzer_PY_CREPE_PLACEHOLDER: Análisis de tono CREPE no implementado para {wav_audio_path}. Devolviendo datos placeholder.")
        if not os.path.exists(wav_audio_path): return {"meanF0Hz": None, "stdDevF0Hz": None, "minF0Hz": None, "maxF0Hz": None, "notes": "Archivo no encontrado."}
        
        placeholder_f0_contour: list[dict] = [] 
        return {
            "meanF0Hz": round(120 + time.time() % 80, 1), 
            "stdDevF0Hz": round(10 + time.time() % 20, 1), 
            "minF0Hz": round(80 + time.time() % 20, 1),
            "maxF0Hz": round(200 + time.time() % 100, 1),
            "notes": "Placeholder F0 data. Integración real de CREPE (o Librosa pyin) pendiente."
        }
    ...
    def process_exercise_submission(
        ...
        warnings: list[str] = []
        pcmData_obj: PcmData | None = None # Para almacenar el objeto PcmData
        ...
        current_timestamp = datetime.datetime.now().isoformat() # Uso correcto de datetime
        ...
        if decoded_info and decoded_info[0] and os.path.exists(decoded_info[0]):
            ...
            # Aquí podrías crear pcmData_obj si es necesario para otros métodos
            # y, sr_loaded = librosa.load(processed_wav_path, sr=sample_rate_val)
            # pcmData_obj = {"samples": y, "sampleRate": sr_loaded}
        else:
            ...
            error_response_data: FinalAnalysisResponse = {
                ...
                "stutternet_analysis": None, "other_stutter_model_analysis": None, # Incluir para consistencia
        ...
        if self.pdf_service:
            try:
                ...
                if not pdf_report_url: 
                    current_warnings = combined_data.get("warnings", []) or [] # Asegurar que es una lista
            ...
            current_warnings = combined_data.get("warnings", []) or [] # Asegurar que es una lista
            current_warnings.append('PdfService no disponible para generar el informe.')
            combined_data["warnings"] = current_warnings
            self.logger.error('PdfService no inyectado/disponible en processAudio.')
        ...
        final_response_data: dict = {
            ...
            "message": "Análisis completado (YAMNet con MediaPipe, otros placeholders).",
            "notes": "Revisar logs. Implementación de CREPE, SER y modelos de tartamudeo específicos pendiente."
        }
